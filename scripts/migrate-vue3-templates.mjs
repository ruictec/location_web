/**
 * One-shot Vue 2 template / Options API syntax migrator for this repo.
 */
import fs from 'fs'
...
function transformTemplate(tpl) {
  ...
  tpl = tpl.replace(/#open=/g, '#active=')
  tpl = tpl.replace(/#close=/g, '#inactive=')
  tpl = tpl.replace(/#open>/g, '#active>')
  tpl = tpl.replace(/#close>/g, '#inactive>')
...
function transformDeepSelectors(css) {
  return css.replace(
    /([^\s{][^{]*?)\s*(>>>|\/deep\/|::v-deep)\s*([^{]+)/g,
...
function transformScript(script) {
  ...
  script = script.replace(
    /\b(\w+)\.\$set\(\s*([^,]+)\s*,\s*(['"][^'"]+['"]|[^,]+)\s*,\s*([^)]+)\)/g,
    (m, obj, target, key, value) => {
      const k = key.trim()
      if (/^['"]/.test(k)) {
        return `${target.trim()}[${k}] = ${value.trim()}`
      }
      return `${target.trim()}[${k}] = ${value.trim()}`
    }
  )
  script = script.replace(
    /from\s+["']vue-count-to["']/g,
    'from "vue3-count-to"'
  )
  script = script.replace(
  ...
  script = script.replace(/:popper-append-to-body=/g, ':teleported=')
  script = script.replace(/\spopper-append-to-body(?=[\s>])/g, '')
...
function transformFile(file) {
  ...
  if (file.endsWith('.vue')) {
    ...
    if (sfc) {
      ...
      const after = transformDeepSelectors(transformScript(sfc.after))
