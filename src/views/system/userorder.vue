...
export default {
  ...
  methods: {
    ...
    delOrder(index) {
      ...
      if (this.orderData[index].status != 5) {
        this.$message({
          message: this.$t("companyorder.tet1"),
          type: "warning",
        });
        return;
      }
    ...
    editTer(index) {
      if (
        ...
        this.orderData[index].upgrade == 1
      ) {
        this.$message({
          message: this.$t("companyorder.tet2"),
          type: "warning",
        });
        return;
      }
      if (this.contrForPrionum == 1 || this.contrForPrionum == 2) {
        this.$message({
          message: this.$t("companyorder.tet3"),
          type: "warning",
        });
        return;
      }
      ...
      } else if (this.orderData[index].status == 4) {
        this.editstatusList = [
          {
            index: 5,
            value: this.$t("floormanagement.close"),
          },
        ];
      }
    ...
    orderDetail(index) {
      this.$router.push({
        path: "/orderdetails",
        query: { orderId: this.orderData[index].id, userorder: true },
      });
    },
  ...
  watch: {
    ...
    "$i18n.locale"() {
      ...
      Object.assign(
        this.$data.editstatusList,
        this.$options.data.call(this).editstatusList
      );
    },
  },
};
