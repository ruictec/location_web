...
<template #dropdown><el-dropdown-menu
          ...
          <!-- 分配aoa -->
          <el-dialog
            :title="$t('otherDev.assigninfoBle')"
            v-model="assign"
            class="edit"
            width="30%"
          ...
          <!-- 批量分配AOA -->
          <el-dialog
            :title="$t('otherDev.assigninfosBle')"
            v-model="assigns"
            class="edit"
            width="30%"
            style="text-align: center"
  ...
  methods: {
    ...
    recycleAoa(index) {
      ...
      if (this.tableData[index].inallot == 2) {
        this.$message({
          message: that.$t("otherDev.BleMes1"),
          type: "warning",
        });
        return;
      }
    ...
    removeAoas() {
      ...
      if (that.multipleSelection.length == 0) {
        that.$message({
          message: that.$t("otherDev.BleMes2"),
          type: "warning",
        });
        return;
      }
      for (let i = 0; i < this.multipleSelection.length; i++) {
        if (this.multipleSelection[i].inallot == 2) {
          this.$message({
            message: this.$t("otherDev.BleMes1"),
            type: "warning",
          });
          return;
        }
    ...
    assignAoa(index) {
      if (this.tableData[index].inallot == 1) {
        this.$message({
          message: this.$t("otherDev.BleMes3"),
          type: "warning",
        });
        return;
      }
    ...
    assignAoas() {
      ...
      if (this.multipleSelection.length == 0) {
        this.$message({
          message: this.$t("otherDev.BleMes4"),
          type: "warning",
        });
        return;
      }
