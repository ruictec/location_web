  ...
  <div style="height: 100%">
    <div class="content">
      <el-container>
        <el-main>
          <div
            class="aoa_input terminal-filter-flow"
          ...
          <div>
            <el-table
              ...
              style="width: 100%; text-align: center; margin-left: 0; z-index: 1"
              max-height="650"
              border
...
<template #dropdown><el-dropdown-menu
                    ...
                    >
                      ...
                      <el-dropdown-item
                        ...
                        ><el-button
                          ...
                          v-if="contrForPrionum == 3 || contrForPrionum == 4"
                          >{{ $t("beacon.assign") }}</el-button></el-dropdown-item><el-dropdown-item
                        style="
                          margin-top: 4%;
                          background-color: rgb(219, 222, 231);
                        "
                      ...
                      >
                        <el-button
                          size="small"
                          class="edits"
                          :disabled="scope.row.inuse == 2"
                          @click="sendDevOtherAlarmOnes('yes', scope.$index)"
                      ...
                      >
                        <el-button
                          ...
                          class="edits"
                          :disabled="scope.row.inuse == 2"
                          @click="sendDevOtherAlarmOnes('no', scope.$index)"
          ...
          <!-- 编辑 -->
          <el-dialog
            :title="$t('otherDev.editinfoAlarm')"
            width="30%"
            class="edit"
            v-model="edit"
            style="text-aling: center"
          ...
          <!-- 批量删除 -->
          <el-dialog :title="$t('terminal.deletemsgs')" v-model="del">
            <el-table
              :data="deleteData"
          ...
          <!-- 分配aoa -->
          <el-dialog
            :title="$t('otherDev.assigninfoAlarm')"
            v-model="assign"
            class="edit"
            width="30%"
          ...
          <!-- 批量分配AOA -->
          <el-dialog
            :title="$t('otherDev.assigninfosAlarm')"
            v-model="assigns"
            class="edit"
            width="30%"
            style="text-align: center"
...
export default {
  ...
  methods: {
    ...
    // 根据设备号触发或取消告警
    sendDevOtherAlarmOnes(val, index) {
      var that = this;
      this.$confirm(
        val == "yes"
        ...
        let data = {
          alarm: val,
          devtype: that.tableData[index].devtype,
    ...
    recycleAoa(index) {
      ...
      if (this.tableData[index].inallot == 2) {
        this.$message({
          message: that.$t("otherDev.AlarmMes1"),
          type: "warning",
        });
        return;
      }
    ...
    removeAoas() {
      ...
      if (that.multipleSelection.length == 0) {
        that.$message({
          message: that.$t("otherDev.AlarmMes2"),
          type: "warning",
        });
        return;
      }
      for (let i = 0; i < this.multipleSelection.length; i++) {
        if (this.multipleSelection[i].inallot == 2) {
          this.$message({
            message: this.$t("otherDev.AlarmMes1"),
            type: "warning",
          });
          return;
        }
    ...
    assignAoa(index) {
      if (this.tableData[index].inallot == 1) {
        this.$message({
          message: this.$t("otherDev.AlarmMes3"),
          type: "warning",
        });
        return;
      }
    ...
    assignAoas() {
      ...
      if (this.multipleSelection.length == 0) {
        this.$message({
          message: this.$t("otherDev.AlarmMes4"),
          type: "warning",
        });
        return;
      }
...
.aoa_input.terminal-filter-flow {
  z-index: 1;
  display: flex;
  flex-wrap: wrap;
  align-items: center;
...
.aoa_input.terminal-filter-flow > .terminal-filter-form.demo-form-inline {
  display: contents !important;
}
.aoa_input.terminal-filter-flow > .terminal-filter-form > .el-form-item,
.aoa_input.terminal-filter-flow .terminal-filter-item {
  width: auto !important;
...
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-form-item__label) {
  width: auto !important;
...
.aoa_input.terminal-filter-flow.is-en .terminal-filter-item :deep(.el-form-item__label) {
  width: auto !important;
  min-width: auto !important;
  flex: 0 0 auto !important;
}
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-form-item__content) {
  margin-left: 0 !important;
  flex: 0 0 auto !important;
  width: auto !important;
  min-width: 0 !important;
}
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-input),
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-select) {
...
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-input__wrapper),
.aoa_input.terminal-filter-flow .terminal-filter-item :deep(.el-select__wrapper) {
  width: 100% !important;
}
...
<style>
/* 对抗 App.vue 的 * { margin: 0 auto }，统一筛选间距 */
