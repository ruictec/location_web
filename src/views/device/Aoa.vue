...
<template #dropdown><el-dropdown-menu
          ...
          <!-- 分配aoa -->
          <el-dialog
            :title="$t('otherDev.assigninfoAoa')"
            v-model="assign"
            class="edit"
            width="30%"
            style="text-align: center"
          ...
          <!-- 批量分配AOA -->
          <el-dialog
            :title="$t('otherDev.assigninfosAoa')"
            v-model="assigns"
            class="edit"
            width="30%"
            style="text-align: center"
  ...
  methods: {
    ...
    recycleAoa(index) {
      ...
      if (this.tableData[index].inallot == 2) {
        this.$message({
          message: that.$t("otherDev.aoames1"),
          type: "warning",
        });
        return;
      }
    ...
    removeAoas() {
      ...
      if (that.multipleSelection.length == 0) {
        that.$message({
          message: that.$t("otherDev.aoames2"),
          type: "warning",
        });
        return;
      }
      for (let i = 0; i < this.multipleSelection.length; i++) {
        if (this.multipleSelection[i].inallot == 2) {
          this.$message({
            message: this.$t("otherDev.aoames1"),
            type: "warning",
          });
          return;
        }
    ...
    assignAoa(index) {
      if (this.tableData[index].inallot == 1) {
        this.$message({
          message: this.$t("otherDev.aoames3"),
          type: "warning",
        });
        return;
      }
        ...
        if (valid) {
          ...
          updateDevOtherByDeveuis(
            ...
            if (res.code == 1001) {
              ...
              that.$refs[assignData].resetFields();
              that.loading = false;
            } else {
              that.$message({
                message: that.$store.state.i18n == "zh" ? res.msg : res.enMsg,
    ...
    assignAoas() {
      ...
      if (this.multipleSelection.length == 0) {
        this.$message({
          message: this.$t("otherDev.aoames4"),
          type: "warning",
        });
        return;
      }
