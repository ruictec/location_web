          ...
          <div class="inputContent">
            <div class="navigation" style="margin-right: 2%">
              <el-tabs
                ...
                :tab-position="
                  $route.path == '/indoor/historical' ? 'right' : ''
                "
              >
                <el-tab-pane
                  class="black"
                  :label="$t('list.Locationquery')"
                  name="location"
                ></el-tab-pane>
                <el-tab-pane
                  class="black"
                  :label="$t('tet.historicaldata')"
                  name="historical"
                ></el-tab-pane>
              </el-tabs>
            </div>
...
export default {
  ...
  methods: {
    handleClick(tab, event) {
      switch (tab.name) {
        case "location":
          this.$router.push("/indoor3d/locationindoor3d");
          this.$store.commit("changeActiveName3D", tab.name);
          break;
        case "historical":
          this.$router.push("/indoor3d/historical3d");
          this.$store.commit("changeActiveName3D", tab.name);
          break;
        default:
          break;
  ...
  computed: {
    activeNames() {
      return this.$store.state.activeName3D;
    }
  },
  watch: {
    activeNames(val) {
