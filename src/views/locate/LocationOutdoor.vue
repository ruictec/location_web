  ...
  methods: {
    ...
    applyVehicleAlarm(deveui, alarm, fromQuery) {
      ...
      if (alarm === undefined || alarm === null || alarm === "") {
        if (fromQuery) {
          this.$set(this.vehicleDoorOpen, deveui, false);
        }
        return;
      }
      const n = Number(alarm);
      if (n === 1) {
        this.$set(this.vehicleDoorOpen, deveui, true);
    ...
    pauseVehicleFlash(deveui) {
      ...
      if (this.vehicleFlashTimers[deveui]) {
        ...
        this.$delete(this.vehicleFlashTimers, deveui);
      }
      this.$delete(this.vehicleFlashLayers, deveui);
    },
    bindVehicleFlashLayer(deveui, layer, src, username) {
      ...
      this.$set(this.vehicleFlashLayers, deveui, {
        layer: layer,
        src: src,
        username: username || "",
        visible: true,
      ...
      if (res.data != "连接成功") {
        ...
        if (data.deveui) {
          ...
          if (data.status != 2) {
            ...
            that.$delete(that.vehicleDoorOpen, data.deveui);
          }
        }
      }
    },
